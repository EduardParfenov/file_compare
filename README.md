...
- `GET /api/jobs/<job_id>` — статус задачи: `processing` (с ключом этапа
  ...
  Клиент опрашивает каждую 1 секунду (первый запрос — сразу после старта)

## Как это работает

1. Загруженные `.docx` конвертируются в Markdown (`python-docx`).
2. Markdown разбивается на блоки (заголовки, абзацы, строки таблиц),
   различия ищутся алгоритмически (`difflib`). Фрагменты `replace`
   дополнительно уточняются по похожести текстов блоков: похожие пары —
   изменения, непохожие — удаления и добавления (так удалённый/добавленный
   текст не маскируется под изменённый).
...
4. Результат отображается side-by-side с подсветкой и синхронным скроллом.
   Внутри изменённых блоков пословная подсветка: добавленные слова — зелёным,
   удалённые — красным, изменённые — жёлтым; удалённые/добавленные блоки
   целиком — красный/жёлтый фон. Таблицы отображаются как HTML-таблицы
   (структурные ячейки, пословная подсветка внутри ячеек, цветные строки
   для добавленных/удалённых).
